...
/*
function moveTitleBgImg(){
	var parid="#titleImage";
	var imgWidth0=$(parid).css("width");
	var reg1=/\d+/;
	var imgWidth=reg1.exec(imgWidth0)[0];
	if(winWidth<imgWidth){
		var cha=(imgWidth-winWidth)/2;
		var offset="-"+cha+"px";
		$(parid).css("margin-left",offset);
	}else{
		$(parid).css("margin-left","0px");
	}
	var winHeight=$(window).height();
	var parHeight=829;
	if(winHeight<parHeight&&winHeight>650){
		var cha=parHeight-winHeight;
		var offset="-"+cha+"px";
		$(parid).css("margin-top",offset);
	}else if(winHeight>829){
		$(parid).css("margin-top",'0px');
	}else if(winHeight<650){
		$(parid).css("margin-top",'179px');
	}
}
*/
...
function keepChildrenMiddle(divid,tags){
	...
	/*var divChildren=$(divid).children();
	for(var i=0;i<divChildren.length;i++){
		var curChild=divChildren[i];
		var curChildWidth_px=curChild.css('width');
		var curChildWidth=reg1.exec(curChildWidth_px);
		var leftdis=(referenceWidth-curChildWidth)/2;
		if(leftdis<0){
			leftdis=0;
		}
		var leftdis_px=leftdis+"px";
		curChiled.css("left",leftdis_px);
	}*/
}